...
void NoximNoC::buildMesh()
{
	...
	for ( i = 0; i < NoximGlobalParams::mesh_dim_x; i++) {
		...
		t[i][j][k]->DW_tag_neighbor[DIRECTION_NORTH](DW_tag_to_south[i][j][NoximGlobalParams::mesh_dim_z-1]);	//������̩��h�o�X��DW_tag
		...
		for ( j=0; j<NoximGlobalParams::mesh_dim_y; j++)
			for ( k=0; k<NoximGlobalParams::mesh_dim_z; k++)
				if (t[i][j][k]->r->local_id == id)
					return t[i][j][k];
	//return false;
	return nullptr; // modified 2024/1008
}
...
{ 
	...
	if (reset.read()) {
	  ...
	  if(( getCurrentCycleNum() % (int) (TEMP_REPORT_PERIOD) ) == 0 )
	  {
		  ...
		  //transient power
		  transPwr2PtraceFile();//��{�b�U�I��power�নptrace file
	      HS_interface->Temperature_calc(instPowerTrace, TemperatureTrace);
	      setTemperature();

		  bool IsEmergency = EmergencyDecision();		//check�{�b�ū׬O�_�W�Lthermal limit, �O�_�n�i�Jemergency mode
...
//		  			avg_cnt_received+=t[x][y][z]->r->cnt_received;
	...
	//}		  			
		  			...
		  			if(!LastIsEmergency)	//�p�L�W��iteration�S��thermal emergency, �N��o����traffic�Χ@�s��traffic quota�p��
		  			{
...
//				cout<<"received_packets:"<<cnt_received_total[0]<<"\t"<<cnt_received_total[1]<<"\t"<<cnt_received_total[2]<<"\t"<<cnt_received_total[3]<<endl;
		...
		for(int z=0; z < NoximGlobalParams::mesh_dim_z; z++) 
			for(int y=0; y < NoximGlobalParams::mesh_dim_y; y++) 
				for(int x=0; x < NoximGlobalParams::mesh_dim_x; x++)
				{							  			
		  			t[x][y][z]->pe->cnt_local = 0;//reset to zero�g�b�o!!���i����
		  			t[x][y][z]->r->cnt_neighbor = 0;//reset to zero�g�b�o!!���i����
		  			t[x][y][z]->r->cnt_received = 0;//reset to zero�g�b�o!!���i����
				}
		  
	  }
	}
	
	if( getCurrentCycleNum() == NoximGlobalParams::simulation_time ) //�����A��steady�p��
	{ 
	...
	for(o=0; o < NoximGlobalParams::mesh_dim_z; o++){
		for(n=0; n < NoximGlobalParams::mesh_dim_y; n++) {
			for(m=0; m < NoximGlobalParams::mesh_dim_x; m++) {
    			...
    			t[m][n][o]->r->stats.power.resetTransientPwr(); //�M���o�q������Power�ֿn
				}
			}
	}
...
//							t[x][y][z]->r->throttle_neighbor = false;
	...
	else if(NoximGlobalParams::throt_type == THROT_DISTRIBUTED)
	{
		...
		for(int z=NoximGlobalParams::mesh_dim_z-1; z >= 0 ; z--) //z�ѤU(�a��heat sink)���W�ˬd, �W���ˬd��L���ɤ~�i�H�s�U���@�_�令emergency
			for(int y=0; y < NoximGlobalParams::mesh_dim_y; y++) 
				for(int x=0; x < NoximGlobalParams::mesh_dim_x; x++)
					{
				...
				for(int x=0; x < NoximGlobalParams::mesh_dim_x; x++)
					{
						...
						{	
								...
								{
									if(zz < NoximGlobalParams::mesh_dim_x*NoximGlobalParams::mesh_dim_y*(NoximGlobalParams::mesh_dim_z-1))	//���]�̾a��heat sink(���h)�����|�W�L, �G���ζi�Jemergency
									{
										t[x][y][zz]->pe->emergency = true;
										t[x][y][zz]->r->emergency = true;
